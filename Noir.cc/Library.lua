...
-- Mobile detection and screen sizing
...
local function GetWindowPosition()
    local Screen = GetScreenSize()
    if IsMobile then
        return UDim2.fromOffset(Screen.X / 2, Screen.Y / 2)
    else
...
function Library:MakeDraggable(Instance, Cutoff)
    ...
    local function OnInputBegan(Input)
        ...
        if isMouse or isTouch then
            local position = isTouch and Input.Position or Vector2.new(Mouse.X, Mouse.Y)
            local ObjPos = Vector2.new(
                position.X - Instance.AbsolutePosition.X,
                position.Y - Instance.AbsolutePosition.Y
            )
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        DisplayFrame:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
            ...
            PickerFrameOuter.Position = UDim2.fromOffset(px, py)
        end)
        ...
        do
            ...
            local function updateMenuPosition()
                local Screen = GetScreenSize()
                local cx = (DisplayFrame.AbsolutePosition.X + DisplayFrame.AbsoluteSize.X) + 4
                local cy = DisplayFrame.AbsolutePosition.Y + 1
                cx = math.clamp(cx, 0, Screen.X - 70)
                cy = math.clamp(cy, 0, Screen.Y - 80)
                ContextMenu.Container.Position = UDim2.fromOffset(cx, cy)
            end
            ...
            function ContextMenu:AddOption(Str, Callback)
                ...
                ConnectInput(Button, function() Callback() end)
            end
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        ToggleLabel:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
            local Screen = GetScreenSize()
            local px = math.min(ToggleLabel.AbsolutePosition.X + ToggleLabel.AbsoluteSize.X + 4, Screen.X - 64)
            ModeSelectOuter.Position = UDim2.fromOffset(px, ToggleLabel.AbsolutePosition.Y + 1)
        end)
...
do
    ...
    function Funcs:AddButton(...)
        ...
        function Button:AddButton(...)
            ...
            self.Outer.Size = UDim2.new(0.5, -2, 0, IsMobile and 28 or 20)
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        function Textbox:OnChanged(Func)
            Textbox.Changed = Func; Func(Textbox.Value);
        end;
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        local LeftSide = Library:Create('ScrollingFrame', {
            ...
            Size = UDim2.new(0.5, -12 + 2, 0, sideHeight + 2);
            CanvasSize = UDim2.new(0, 0, 0, 0);
        ...
        function Tab:AddGroupbox(Info)
            ...
            local BoxOuter = Library:Create('Frame', {
                ...
                BorderColor3 = Library.OutlineColor;
                BorderMode = Enum.BorderMode.Inset;
                Size = UDim2.new(1, 0, 0, sideHeight + 2);
                ZIndex = 2;
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                local Container = Library:Create('Frame', {
                    BackgroundTransparency = 1;
                    Position = UDim2.new(0, 4, 0, IsMobile and 28 or 20);
                    Size = UDim2.new(1, -4, 1, IsMobile and -28 or -20);
